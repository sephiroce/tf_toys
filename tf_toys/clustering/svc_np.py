...
@author: josiahw
@url: https://github.com/josiahw/SimpleSVClustering/blob/master/SimpleSVC.py
"""
import numpy
import numpy.linalg
import time
...
class SimpleSVClustering:
  ...
  def _getAllClasses(self, X):
    ...
    while len(unvisited):
      ...
      while i < len(c) and len(unvisited):
        ...
        unvisitedNew = []
        for j in unvisited:
          (c if self._checkClass(X[c[i], :], X[j, :]) else unvisitedNew).append(
            j)
  ...
  def fit(self, X):
    ...
    if self.verbose:
      print(
      "Number of support vectors: %d"%len(self.a))
  ...
  def _predict(self, X):
    ...
    for i in range(len(X)):
      ...
      for j in range(len(self.sv)):
        clss[i] -= 2 * self.a[j] * self.kernel(
          *((self.sv[j, :], X[i, :]) + self.kargs))
...
if __name__ == '__main__':
  ...
  data, labels = sklearn.datasets.make_circles(n_samples=400, factor=.3, noise=0.025)
  ...
  clss.fit(data)
  print("elapsed time to fit: %.3f"%(time.time() - start_time))
  ...
  print("elapsed time to predict: %.3f" % (time.time() - start_time))
